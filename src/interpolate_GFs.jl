...
"""
function spectral_function_value(ω::Number, operator_type::OperatorType, x::AbstractVector, y::AbstractVector, C0::Real, float_type::DataType=Double64; η::Real = 1.e-5, toreverse::Bool=true, init_func::Function= z -> zero(Complex{float_type}))
    x, y = toNevanlinnadata(x, y, operator_type, float_type)
    if toreverse == true
        x = reverse(x)
        y = reverse(y)
    end
    C0 = convert(float_type, C0)
    append!(x, one(float_type)im * η)
    append!(y, one(float_type)im * η * (one(float_type) - η) * C0)
...
"""
function spectral_function(operator_type::OperatorType, x::AbstractVector, y::AbstractVector, C0::Real, float_type::DataType=Double64; ωmax::Real = 4π, Nω::Integer = 500, η::Real = 1.e-5, toreverse::Bool=true, init_func::Function= z -> zero(Complex{float_type}))
    L = 2*ωmax
    ωlist = convert(Vector{float_type}, (-Nω/2:Nω/2-1)*L/Nω)
    Alist = map(ω -> spectral_function_value(ω, operator_type, x, y, C0, float_type; η, toreverse, init_func), ωlist)
    return ωlist, Alist
end