...
\begin{equation}\label{eq:inv-schur-function-recursion-anpther-s}
	\tilde{s}_j(\tilde{z}) 
	= \frac{\C_j + \xi(\tilde{z}, \Y_j) \tilde{s}_{j+1}(\tilde{z}) }
		{1 + \C_j^* \xi(\tilde{z}, \Y_j) \tilde{s}_{j+1}(\tilde{z})}
\end{equation}
\begin{proof}
	...
	\begin{equation}
		...
		= \frac{\C_j + z \tilde{s}_{j+1}(\xi^{-1}(z, \Y_j)) }
			{1 + \C_j^* z \tilde{s}_{j+1}(\xi^{-1}(z, \Y_j))}
	\end{equation}
...
\begin{align}
	...
	&s_{N-1}(z;\C_{N-1}) 
		= \frac{\C_{N-1} + \xi(z,\Y_{N-1})s_N(z)}
			{1 + \C_{N-1}^*\xi(z,\Y_{N-1})s_N(z)} \\
	\label{eq:schur-interpolation-2}
	&s_j(z;\C_j,\C_{j+1},\dots \C_N) 
		= \frac{\C_j + \xi(z,\Y_{j})s_{j+1}(z;\C_{j+1},\dots \C_N)}
			{1 + \C_j^* \xi(z,\Y_{j})s_{j+1}(z;\C_{k+1},\dots \C_N)}
\end{align}
where $j = N-2, N-3, \dots, 1, 0$ and $s_0(z) \equiv s(z)$. 
In \cref{eq:schur-interpolation-1}, 
