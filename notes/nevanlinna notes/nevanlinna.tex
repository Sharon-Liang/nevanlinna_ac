...
parameters and $|\gamma_j| \leq 1$. This gives a continued fraction 
expansion of the schur function $s(z)$ by repeatedly using the fact 
that:
\begin{equation}\label{eq:inv-schur-function-recursion-frac}
	s_{j}(z) = \gamma_j + \frac{1 - |\gamma_j|^2}
		{\gamma_j^* + \frac{1}{zs_{j+1}(z)}}
\end{equation}
which gives:
\begin{equation}\label{eq:schur-fraction-expansion}
	s(z) \equiv s_0(z) = \gamma_0 + \frac{1 - |\gamma_0|^2}
		{\gamma_0^* + \frac{1}
...
\subsubsection{Schur interploation algorithm}
\label{subsubsec:schur-interpolation-problem}
...
The inverse relation of \cref{eq:recursive-s} is
...
\subsubsection{The determination of Schur parameters}
\label{subsubsec:determination-of-schur-parameters}
...
\end{equation}
\begin{align}\label{eq:explicit-gamma-2}
\begin{split}
...
\begin{align}\label{eq:explicit-gamma-jp1}
\begin{split}
...
Similar to \cref{eq:inv-schur-function-recursion-frac},
We can also constract a continued fraction 
expansion form of $s(z)$ by repeatedly using the fact 
that:
\begin{equation}\label{eq:inv-s-s-recursion-frac}
	s_{j}(z) = \gamma_j + \frac{1 - |\gamma_j|^2}
		{\gamma_j^* + \frac{1}{\xi(z,\Y_j) s_{j+1}(z)}}
\end{equation}
which gives:
\begin{equation}\label{eq:s-s-fraction-expansion-fit}
	s(z) \equiv s_0(z) = \gamma_0 + \frac{1 - |\gamma_0|^2}
		{\gamma_0^* + \frac{1}
		{\xi(z,\Y_0)\gamma_1 + \frac{\xi(z,\Y_0) (1 - |\gamma_1|^2)}
		{\gamma_1^* + \frac{1}{\xi(z,\Y_1) \gamma_2 + \cdots}}
		}
		}
\end{equation}


\subsubsection{Interpolate site by site}
\label{subsubsec:interpolate-site-by-site}

Here we review the interpolation problem in a more explicit 
way, that is to interpolate site by site. Here we use the 
recursion relation \cref{eq:inv-schur-function-recursion-another-s}, 
and our target function is $s(z) \equiv s_1(z)$.

If we have only one site $(\Y_1, \C_1)$, according to 
\cref{eq:inv-schur-function-recursion-another-s}, $s_1(z)$ 
has the following form:
\begin{equation}
	s_1(z; Y_1, \C_1)
	= \frac{\gamma_1 + \xi(z, \Y_1) s_{2}(z) }
		{1 + \gamma_1^* \xi(z, \Y_1) s_{2}(z)}
\end{equation}
where $\gamma_0 = s_0(\Y_0) = \C_0$, and $s_2(z)$ is an 
arbitrary schur function.

When another site $(\Y_2, \C_3)$ is added. $s_2(z)$ is 
required to have the following form:
\begin{equation}
	s_2(z)
	= \frac{\gamma_2 + \xi(z, \Y_2) s_{3}(z) }
		{1 + \gamma_2^* \xi(z, \Y_3) s_{3}(z)}
\end{equation}
where $s_3(z)$ is an arbitrary schur function. 
The Schur parameter $\gamma_2$ can be determined as 
shown in \cref{subsubsec:determination-of-schur-parameters}, 
whose explicit form is \cref{eq:explicit-gamma-2}. 
We can see that $s_2(z) = s_2(z; \Y_1, \C_1, \Y_2, \C_2)$ 
and $s_1(z) = \frac{a_2(z) s_3(z) + b_2(z)}{c_2(z) s_3(z) + d_2(z)}$, 
with:
\begin{equation}\label{eq:explicit-ancd2}
	\left(
		\begin{matrix}
			a_2(z) & b_2(z) \\
			c_2(z) & d_2(z)
...
Suppose we already has the $n$-th order of interpolation:
\begin{equation}
	s_1(z) = \frac{a_n(z) s_{n+1}(z) + b_n(z)}
		{c_n(z) s_{n+1}(z) + d_n(z)}
\end{equation}
Adding another point $(\Y_{n+1}, \C_{n+1})$ would make the 
arbitrary Schur function $s_{n+1}(z)$ to aquire a structure 
\begin{equation}
	s_{n+1}(z)
	= \frac{\gamma_{n+1} + \xi(z, \Y_{n+1}) s_{n+2}(z) }
		{1 + \gamma_{n+2}^* \xi(z, \Y_{n+2}) s_{n+2}(z)}
\end{equation}
and $\gamma_{n+1}$ is calculated by \cref{eq:explicit-gamma-jp1} 
and $s_{n+2}(z)$ is an arbitrary Schur function.
 

\subsubsection{Pick matrix}
\label{subsubsec:pick-matrix}
...
expansion form of $\theta(z)$ can be constracted by 
repeatedly using the fact that:
\begin{equation}\label{eq:inv-theta-recursion-frac}
	\theta_{j}(z) = \gamma_j + \frac{1 - |\gamma_j|^2}
		{\gamma_j^* + \frac{1}{h(z,\Y_j) \theta_{j+1}(z)}}
\end{equation}
which gives:
\begin{equation}\label{eq:theta-fraction-expansion-fit}
	\theta(z) \equiv \theta_1(z) = \gamma_1 + \frac{1 - |\gamma_1|^2}
		{\gamma_1^* + \frac{1}
		{h(z,\Y_1)\gamma_2 + \frac{h(z,\Y_1) (1 - |\gamma_2|^2)}
		{\gamma_2^* + \frac{1}{h(z,\Y_2) \gamma_3 + \cdots}}
		}
		}
\end{equation}
